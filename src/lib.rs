...
fn std_dyad<T>(data: &mut [T], p: usize, mut is_less: impl Fn(&T, &T) -> bool) -> (usize, usize) {
    data.swap(0, p);
    let (head, tail) = data.split_first_mut().unwrap();
    let tmp = unsafe { ManuallyDrop::new(ptr::read(head)) };
    let pivot = &*tmp;

    let u = partition_in_blocks(tail, pivot, &mut is_less);
    (u+1, u+1)
}
